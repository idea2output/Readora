...
export async function searchRelevantChunks(
  ...
  const { data: existingChunks } = await supabase
    .from('book_chunks')
    .select(
      'id, book_id, chapter_id, sequence_number, title, content'
    )
    .eq('book_id', bookId)
    .limit(limit);
  ...
  // Auto-index if chunks don't exist
...
export async function askTheBook(
  ...
  if (!chunks || chunks.length === 0) {
    return {
      answer:
        'I could not find enough text in this book to answer your question accurately.',
      citations: [],
      model: 'readora-rag',
    };
  }
  ...
  try {
    await supabase
      ...
      .insert({
        ...
        estimated_cost:
          completion.inputTokens * 0.000001 +
          completion.outputTokens * 0.000002,
      });
  } catch (_) {
    // Usage logging should never break the AI response.
  }
