...
export async function POST(request: Request) {
  try {
    ...
    if (!gutendexId) {
      return NextResponse.json({ error: 'Could not resolve Gutenberg ID for this book' }, { status: 400 });
    }
    ...
    if (!downloadedContent) {
      return NextResponse.json({ error: 'Failed to download content from Gutenberg mirrors.' }, { status: 502 });
    }
    ...
    if (insertErr) {
      return NextResponse.json({ error: `Insert error: ${insertErr.message}` }, { status: 500 });
    }
