...
export async function POST(request: Request) {
  try {
    const { userId, action, role, status } = await request.json();
    if (!userId) {
    ...
    if (action === 'change_role' && role) {
      ...
      await logAuditEvent('USER_ROLE_CHANGED', 'user', userId, { newRole: role });
    ...
    if (action === 'toggle_status' && status) {
      ...
      await logAuditEvent('USER_SUSPENDED', 'user', userId, { newStatus: status });
      return NextResponse.json({ success: true, message: `Account status updated to ${status}` });
    }

    return NextResponse.json({ error: 'Invalid action' }, { status: 400 });
  } catch (error: any) {
    return NextResponse.json({ error: error.message || 'User action failed' }, { status: 500 });
  }
}
